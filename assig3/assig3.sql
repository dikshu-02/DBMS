...
REM ** creation of customer table **

create table customers (
cid int CONSTRAINT pk_cust PRIMARY KEY ,
fname varchar2(20),
lname varchar2(20)
);

REM ** creation of products table **

create table products (
pid varchar2(20) CONSTRAINT pk_prod PRIMARY KEY,
flavor varchar2(20) ,
food varchar2(20) ,
price Decimal(5,2)
);

REM ** creation of Receipts table **

 create table Receipts (
 rno Number(5) CONSTRAINT pk_receipts PRIMARY KEY,
 rdate date,
 cid int Constraint fk_rec references customers(cid)
 );


REM ** creation of item_list table **

 create table item_list (
 rno Number(5) CONSTRAINT fk_it_list references Receipts(rno),
 ordinal int ,
 item varchar2(20) CONSTRAINT fk2_it_list references products(pid),
 CONSTRAINT pk_it_list PRIMARY KEY(rno,ordinal)
 )
 ;
...
REM **Display the food details that is not purchased by any of customers**
 select * from products where pid not in (select item from item_list);
 select item from item_list where item='20-BC-C-10';

REM **Show the customer details who had placed more than 2 orders on the same date **
select * from customers where cid in 
(select cid from (select cid,rdate 
from Receipts group by(cid,rdate) having count(*)>2)
);

REM **Display the products details that has been ordered maximum by the customers. (use ALL) **
 select * from products where pid in 
( select item from item_list group by(item) having count(item) >= 
all(select count(item) from item_list group by (item))
);
...
select count(distinct(rno)) from products p, item_list i where p.pid=i.item and price>(select avg(price) from 
products ip where p.food=ip.food group by food);
...
select customers.cid,fname,lname ,rno,rdate from customers JOIN Receipts  ON
customers.cid=Receipts.cid and rdate=last_day(rdate);
...
select distinct(i.rno) , sum(p.price) from item_list i join products p on i.item=p.pid group by(i.rno)
having sum(price)>25 and count(*)=5 and i.rno in (
select rno from item_list where item in (
select pid from products where food='Twist')
...
select r.rno, c.cid,c.fname,c.lname,i.item from Receipts r join item_list i on r.rno=i.rno join customers c on r.cid=c.cid 
where i.item in (
select item from item_list group by(item) having count(*) =
...
select c.cid,c.fname,c.lname , r1.rno from customers c join Receipts r1 on(r1.cid=c.cid)
where r1.rno=(
select r2.rno from Receipts r2 join item_list i on(r2.rno=i.rno) join products p on(p.pid=i.item)
...
REM ** Display the food that has the same flavor as that of the common flavor between the **
